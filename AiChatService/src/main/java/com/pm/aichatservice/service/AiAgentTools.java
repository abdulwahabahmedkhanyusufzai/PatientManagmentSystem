...
public class AiAgentTools {
  ...
  public Function<PatientListRequest, PatientListResponse> getPatientList() {
    return request -> {
      com.pm.patientservice.grpc.PatientListResponse response = patientStub.getPatients(Empty.newBuilder().build());
      String patients = response.getPatientsList().stream()
          .map(p -> String.format("ID: %s, Name: %s, Email: %s", p.getId(), p.getName(), p.getEmail()))
